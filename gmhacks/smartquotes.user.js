...
// based on "Smart Firefox" by Jonathan Fenocchi
...
function filterChars(formatted) {
    ...
    var SINGLE_QUOTES = new RegExp( '^\'|([^' + BASE_CHAR + DIGIT + 
				    '])\'\\b', 'g' );
	...
	temp[ count ] = RegExp.$1;
	formatted = formatted.replace(RegExp.$1,DELIM_CHAR+count+DELIM_CHAR);
	count++;
    }
    ...
    formatted=formatted.replace(/(\[(code|html|php)\])([\w\W\s]+?)(\[\/\2\])/gi,
        function(s){
	    s = s.replace(/&#8216;/g, "'");
	    s = s.replace(/&#8217;/g, "'");
	    s = s.replace(/&#8220;/g, '"');
