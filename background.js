...
var excludes = [
	"https://mail.google.com/*", 'http://b.hatena.ne.jp/*', 'http://www.facebook.com/plugins/like.php*', 'http://api.tweetmeme.com/button.js*',
];

function init() {
	if (!localStorage['settings']) {
		var defaultSettings = {
			extension_path: chrome.extension.getURL(''), display_message_bar: true, exclude_patterns: localStorage['exclude_patterns'] || '',
		};
...
function loadLocalSiteinfoCallback(data) {
	...
	if (!cache[url]) {
		siteinfo[url] = {
			url: url, expire: new Date().getTime() - 1, info: reduceWedataJSON(data),
		};
	...
	SITEINFO_IMPORT_URLS.forEach(function (url) {
		if (opt.force || !cache[url] || (cache[url].expire && new Date(cache[url].expire) < new Date())) {
			var callback = function (res) {
				...
				siteinfo[url] = {
					url: url, expire: new Date().getTime() + CACHE_EXPIRE, info: info,
				};
				cache[url] = siteinfo[url];
				localStorage['cacheInfo'] = JSON.stringify(cache);
				if (opt.callback) {
