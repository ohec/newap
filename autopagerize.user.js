...
(function () {
  ...
  var SITEINFO_IMPORT_URLS = ["http://wedata.net/databases/AutoPagerize/items.json",
							  "http://localhost/ap/customsites.json"];
  var SITEINFO             = [
...

	{
	  ...
	  "exampleUrl": "https://rlsbb.unblocked.bid/category/tv-shows/",
	  "url": "^https://rlsbb\\.unblocked\\.bid/category/tv-shows/",
	  "pageElement": "//div[@class='post']",
	  "nextLink": "//a[(text()='Older Entries')]",
	}, {
	  ...
	  "url": "^http://(.*)300mbunited(.*)",
	  "nextLink": "//div[@class='nav-previous']/a",
	  "pageElement": "//div[@id='content']",
	  "exampleUrl": "http://www.300mbunited..me",
	},
	/* samplehttps://plugins.jetbrains.com/new-or-updated/idea? http://(.*).google.+/(search).+
	 https://plugins.jetbrains.com/new-or-updated/idea?correctionAllowed=false&search=&offset=20&max=10&orderBy=update+date&title=New+or+Updated
	 {
  ...
  var MICROFORMAT          = {
	url: ".*",
	nextLink: "//a[@rel=\"next\"] | //link[@rel=\"next\"] | //span[@id=\"olderEntries\"]/a || //a[@class='nextLink'] || //a[(text()='Next Page>')] || //a[(text()='Next Page')] || //div[@class='nav-previous']/a",
  ...
  AutoPager.prototype.getPageElementsBottom = function () {
	try {
	  ...
	  var bs    = elems.map(function (i) {
		return getElementBottom(i);
	  });
  ...
  AutoPager.prototype.request = function () {
	...
	if (Extension.isFirefox()) {
	  extension.postMessage("get", {
		url: this.requestURL, fromURL: location.href, charset: document.characterSet,
	  }, function (res) {
  ...
  AutoPager.prototype.getNextURL = function (xpath, doc, url) {
	...
	if (nextLink) {
	  var nextValue = nextLink.getAttribute("href") || nextLink.getAttribute("action") || nextLink.value;
  ...
  if ((/^https?:\/\/www.youtube.com\/results.+/).test(location.href)) {
	var youtubeSearchShowThumbnalFilter = function (nodes) {
	  nodes.forEach(function (i) {
		Array.prototype.slice.call(i.querySelectorAll("img[data-thumb]")).forEach(function (i) {
...
// utility functions.
  function getElementsByXPath(xpath, node) {
	var nodesSnapshot = getXPathResult(xpath, node, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE || 7);
	var data          = [];
  ...
  function getFirstElementByXPath(xpath, node) {
	var result = getXPathResult(xpath, node, XPathResult.FIRST_ORDERED_NODE_TYPE || 9);
